class CourseDescriptionsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @course_description.save
        ...
        format.json { render action: 'show', status: :created, location: @course_description }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to course_descriptions_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def course_description_params
      params.require(:course_description).permit(:name, :code)
    end
end
